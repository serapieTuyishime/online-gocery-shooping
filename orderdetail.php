...
?>
 ...
 <div class="card">
            ...
            <div class="card-header">
             ...
             <center><h2>Groceries shop</h2><p style="font-size: 20px">Musanze city</p></center>
         </div>
                ...
                $query = "SELECT b.product_name,a.date,a.qty,a.price,a.total FROM tbltransac a,tblproducts b WHERE a.product_code=b.product_code AND a.transac_code='".$_GET['id']."' GROUP BY a.product_code";
                    $result = mysqli_query($db, $query) or die (mysqli_error($db));
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo '<td>'. $row['date'].'</td>';
                            echo '<td>'. $row['qty'].'</td>';
                            echo '<td>Frw '. $row['price'].'</td>';
                            echo '<td>Frw '. $row['total'].'</td>';
                           /* echo '<td>  ';
...
?>
        ...
        <!--   <h5 align="right" style="margin-right: : 150px">Total Price :  Frw <?php echo $zz; ?></h5> -->
          </div>
