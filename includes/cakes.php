...
{
  $av = $_POST['av']; //hidden quantity from database
  $qq = $_POST["quant"]; //quantity by customer input
  if ($av > 0) 
  {

    // if the store is bigger than input or when they are equal
    if ($av > $qq || $av == $qq)  
    {

      // check if the cart was previously available
      if (isset($_SESSION["cart"])) 
      {
        ...
        if (!in_array($_GET["id"], $itemarrayid)) {
          ...
          echo "<script>alert('Item Already Added but you can delete the previously added item')</script>";
          echo "<script>window.location = 'cart.php'</script>";
        }
      }
      else
...
<div class="col-lg-3">
  <div class="card mb-3">
    <div class="card-body">
      <form method="post" action="indexx.php?action=add&id=<?php echo $row["product_code"]; ?>">
        <center><img src="image/<?php echo $row['product_image']; ?>" style="width: 150px">
         <h4 class="text-info"><?php echo $row["product_name"]; ?></h4>
         <h5 class="text-info">Available Qty:(<?php echo $row["quantity"]; ?>)</h5>
         <h4 class="text-danger">frw <?php echo $row["price"]; ?>.00</h4>
