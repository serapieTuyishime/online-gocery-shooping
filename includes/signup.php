...
{
	header("Location: ../register.php?error= We only accept age under 100 and over 10");
	exit();
}
...
if ($password !== $passcon) {
	...
	exit();
		...
		if ($resultCheck > 0) {
			...
			$sql = "INSERT INTO `tblcustomer`(`C_FNAME`, `C_LNAME`, `C_AGE`, `C_ADDRESS`, `C_PNUMBER`, `C_GENDER`, `C_EMAILADD`, `username`, `password`)VALUES(?,?,?,?,?,?,?,?,?)";
			mysqli_stmt_execute($stmt);
			if (!mysqli_stmt_prepare($stmt,$sql)) {
			header("Location: ../register.php?error=sqlerror");
			exit();
			...
			mysqli_stmt_bind_param($stmt,"sssssssss",$C_FNAME,$C_LNAME,$C_AGE,$C_ADDRESS,$C_PNUMBER,$C_GENDER,$C_EMAILADD,$username,$hashedPwd);
			mysqli_stmt_execute($stmt);
